resFunLog = function(TP, sigmaH2muH, Vmax2SigmaH, muH2Dmin, D02Dmin, P02Pc, V02Vmax,
                  rhoH, muS, alpha, beta, deltaNG, Kp, muT, sigmaT, rhoT) {
...
# resFunLog = function(TP, parm, init) {
  ...
  Temp <- numeric(length = TP)      #Temperature vector
  ...
  while (t < TP + 1) {
    ...
    et_s1 <- 6.11*exp(17.3*Temp[t]/(237.3+Temp[t]))
    
    # #PET in Ml/year, where Area1 is the area of the reservoir and Flow is the streamflow serie
    ET <- 0.00409*et_s1*Area1/1000*12
    ...
    Temp[t+1] <- max((sigmaT*(1-rhoT^2)^0.5)*rnorm(1) + muT,0)
    t <- t + 1
