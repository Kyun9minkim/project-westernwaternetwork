...
histCALA_series = reshape(histCALA_monthly, [12*num_years, 100]); % (240 x 100)

qCO_series = reshape(qCO_monthly, [12*num_years, 100]); % (240 x 100)
qRG_series = reshape(qRG_monthly, [12*num_years, 100]); % (240 x 100)
qCALA_series = reshape(qCALA_monthly, [12*num_years, 100]); % (240 x 100)
...
group_labels = [repmat({'CO-RG Historical'}, 100, 1); repmat({'CO-RG Generated'}, 100, 1); 
                repmat({'CO-CALA Historical'}, 100, 1); repmat({'CO-CALA Generated'}, 100, 1); 
                repmat({'RG-LACA Historical'}, 100, 1); repmat({'RG-LACA Generated'}, 100, 1)];
